...
def passes_quality_filter(text: str) -> bool:
    ...
    letters_count = sum(
        1 for _ in LETTER_RE.finditer(text)
    )
    ...
    arabic_count = sum(
        1 for _ in ARABIC_RE.finditer(text)
    )

    if arabic_count / letters_count < MIN_ARABIC_RATIO:
        return False
    ...
    try:
        ...
        while accepted < TARGET_SAMPLES:
            ...
            for text, in rows:
                ...
                if TEST_STOP_AFTER_SHARD and shard_number == 1:
                    print(
                        "TEST STOP: "
